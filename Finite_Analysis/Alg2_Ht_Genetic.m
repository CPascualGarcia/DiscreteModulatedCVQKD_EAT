...
% extremals of the function) using a perturbed dual
% IN
...
%  kappa    - perturbation vector for the dual
%
% OUT
...
function [AlgVars] = Alg2_Ht_Genetic(GammaRaw,rho,xi,InitialVars,kappa)
...
cvx_begin quiet
    ...
    % Perturbation term
    Pert = [norm(nu,1);norm(nu,2);norm(nu,Inf)];

    maximize (dot(probdist,nu)-epsilon*dot(ones(1,S+4),mu) - kappa*Pert)
...
% Renormalization (actually not necessary bc. there's no postselection)
p_passzx = sum(Pzx)/4;
Pzx      = Pzx/p_passzx;
