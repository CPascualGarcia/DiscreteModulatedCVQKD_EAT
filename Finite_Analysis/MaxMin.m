...
%%%
%%% INPUT: y - variables of the dual maximization
%%%        
%%% OUTPUT: MaxMinf - Difference between the bounds: Maxf - Minf
...
if Max1 == max(yPE) && Min1 == min(yPE) 
%IndMax <= length(yPE) && IndMin <= length(yPE)
    cvx_precision best
    cvx_begin quiet
        variable x
...
elseif Max1 == max(ytom) && Min1 == min(ytom)
%IndMax > length(yPE) && IndMin > length(yPE)
    cvx_precision best
    cvx_begin quiet
        variable x
...
% The extremals belong to different sets
    ...
    if Max1 == max(yPE) % The max. is related to PE rounds
    %IndMax <= length(yPE)
        Ppe = x; Ptom = 1-x;
